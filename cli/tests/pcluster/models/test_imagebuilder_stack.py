...
@pytest.mark.parametrize(
    "describe_stack_response, describe_stack_resources_side_effect",
    [
        (
            {"StackName": "pcluster2"},
            [
                ...
                },
                {
                    "StackResourceDetail": {
                        "PhysicalResourceId": "arn:aws:imagebuilder:us-east-1:xxxxxxxxxxxx:image"
                        "/parallelclusterimagerecipe-7473d4s2otwynhxr/2.10.1/2"
                    }
                },
            ],
        )
...
def test_update_info(mocker, describe_stack_response, describe_stack_resources_side_effect):
    mocker.patch("common.aws.aws_api.AWSApi.instance", return_value=DummyAWSApi())
    mocker.patch(
        "common.boto3.cfn.CfnClient.describe_stack_resource",
        side_effect=describe_stack_resources_side_effect,
    )
    mocker.patch(
        "common.boto3.cfn.CfnClient.describe_stack",
        return_value=describe_stack_response,
    )

    initial_stack_data = {"StackName": FAKE_IMAGEBUILDER_STACK_NAME}
    imagebuilder_stack = ImageBuilderStack(initial_stack_data)
    assert_that(imagebuilder_stack._stack_data).is_equal_to(initial_stack_data)
    assert_that(imagebuilder_stack._image_resource).is_equal_to(describe_stack_resources_side_effect[0])

    imagebuilder_stack.update_stack_info()
    assert_that(imagebuilder_stack._stack_data).is_equal_to(describe_stack_response)
    assert_that(imagebuilder_stack._image_resource).is_equal_to(describe_stack_resources_side_effect[1])
